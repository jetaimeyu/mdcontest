...
class Redis {
    ...
    {
    	/*
    	*liuj-ad 整合C方法，配置redis
    	 */
    	$config = [
	        'host'          => C('SESSION_REDIS_HOST') ? C('SESSION_REDIS_HOST') : '127.0.0.1',
	        'port'          => C('SESSION_REDIS_PORT') ? C('SESSION_REDIS_PORT') : 6379,
	        'password'      => C('SESSION_REDIS_PASSWORD') ? C('SESSION_REDIS_PASSWORD') : '',
	        'timeout'       => C('SESSION_TIMEOUT')    ? C('SESSION_TIMEOUT') : 0,
	        'persistent'    => C('SESSION_PERSISTENT') ? C('SESSION_PERSISTENT') : true,
			'expire'		=> C('SESSION_EXPIRE')     ? C('SESSION_EXPIRE') : 3600,
			'session_name'	=> C('SESSION_NAME_PREFIX')? C('SESSION_NAME_PREFIX') : '',
