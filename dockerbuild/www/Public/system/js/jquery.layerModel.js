...
; (function($) {
	...
	var methods = {
		...
		init: function(data, options) {
			var defaults = {
				center: true,locked: true,fixed: true,drag: true,zIndex: 1099,opacity: "0.5",title: "系统提示",staySame: false,width: 0,height: 0,timer: 0,bgColor: "#e5dfda",left: 350,top: 100,head: true,isClose: true,shake: false,blurClose: false,close: null,init: null
			};
		...
		createContainer: function(data, settings, generateId) {
			...
			if (settings.height > 0) {
				...
				s.container.width(tempWidth);
				$("#" + s.o.warpperContent + generateId).width(tempWidth);
		...
		close: function(data, settings, generateId) {
			if($('.layerModel_wrapper').length == 1){
